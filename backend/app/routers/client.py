...
async def _compter_taches(db: AsyncSession, projet_id: int, statut: Optional[StatutTache] = None) -> int:
    query = select(func.count(Tache.id)).where(Tache.projet_id == projet_id)
...
async def get_client_statut(
    ...
    total_taches = await _compter_taches(db, projet.id)
    taches_terminees = await _compter_taches(db, projet.id, StatutTache.TERMINE)
...
async def get_client_avancement(
    ...
    for statut in StatutTache:
        stats[statut.value] = await _compter_taches(db, projet.id, statut)
